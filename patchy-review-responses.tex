...
We thank the editor and the reviewers for their helpful comment. We
have substantially revised our paper in response ot these comments,
...
coded an individual level model of migration and selection. 
We have done a large number of simulations, and compared the results to the theory;
...
\begin{itemize}
  ...
  \item We have also improved our discussion of the limits to the theory,
    ...
    we were \gc{originally} running \gc{simulations} in regions where $s_m$ was too small for the theory to apply,
...
\reply
We don't know of even a poor approximation to the time to reach migration-selection equilibrium,
unfortunately. However, we now pointed out in the section ``\nameref{ss:assumptions}'' that we're missing this point,
and discuss there how the time lag to reach this equilbrium contributes to discrepancies between theory and simulations.

\gc{seems to graham that we should spread to migration-selection
  equilibrium as the fisher-kpp. Suggests that patch width$/sqrt{s}
  \sigma$ wouldn't be a bad guess.}
...
There are also other assumptions in Barton's paper that it's applicability unclear.
...
in this draft we've replaced this figure with a different one (so
disgarded this comment).
...
\reply
We've made this more clear here 
\llname{rr:longtailed}.
