...
\begin{itemize}
  ...
  \item In response to a request by the PLoS Genetics staff to ``either amend the supplementary material to include the referenced data or remove as many of the references [to ``results not shown''] as possible.''
    ...
    ''
    The additional experimentation is for an upcoming paper, 
    but one that is in an early enough draft that it does not seem useful to cite it.
    We are happy to remove the sentence if that would be better.
...
Page and line numbers in the response to reviewers below refer to this document.
