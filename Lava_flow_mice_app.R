...
plot(R, prob.parallel,type="l",lwd=3, xlab="", ylab="",cex.lab=1.3,cex.axis=1.3,log="x") 
mtext(side=2,line=2.5,"prob parallel mutation",cex=1.3)
mu=1e-5
prob.parallel <- mu*A/(mu*A + (1/char.length[1])* exp(-R/char.length[1]))  ##assuming sigma=1
...
mtext(side=1,line=2.5,"Distance between patches (km)",cex=1.3)
...
mtext(side=2,line=2.5,"Shared Haplotype Length (cM)",cex=1.3)
mtext(side=1,line=2.5,"Distance between patches (km)",cex=1.3)
