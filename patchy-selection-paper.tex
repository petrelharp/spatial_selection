...
it seems helpful to step back and review the assumptions we have made and the parameter regime
we expect the results to apply to.
For the mutation rate,
our results will hold best if $s_m \approx s_p$,
if the patch diameter is large relative to $\sigma/\sqrt{\min(s_m,s_p)}$,
the patch is not too far from circular.
If $s_m \ll s_p$ then \eqref{eqn:mutrate} will underestimate the rate,
and conversely if $s_m \gg s_p$.
