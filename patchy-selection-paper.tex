...
Suppose that there is a constant outflux of migrant families of rate $\gamma(x)$ from each point $x \in \partial B(r_0)$.
...
\begin{align}\label{eqn:migrate_integral}
  \migrate(y) &= \int_{\partial B(r_0)} \gamma(x) h(x,y) f(A) dx .
\end{align}
...
\begin{align}
  \migrate(y) \approx q(y) \times \frac{ s_p  }{ \alpha \xi^2 } 
