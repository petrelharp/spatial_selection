...
\begin{align}
  ...
  &\approx C \frac{ \rho s_p  }{ \xi^2 s_m } \left( |y| \sqrt{2 s_m}/\sigma \right)^{-(d-1)/2} \exp( - |y| \sqrt{2 s_m} / \sigma) .
\end{align}
...
then adaptation is mostly parallel for patches separated by gaps larger than $R/\sigma > (7-\log(s_m))/\sqrt{2s_m}$.
If the selective pressure is fairly strong (say, $s_m=.05$),
then for migration to be slower than mutation,
the distance between patches must be at least 31.6 times the dispersal
distance (i.e.\ $R/\sigma > 31.6$) for convergence to occur.
If $s_m$ is much smaller, say $s_m = .001$, 
then the patches must be separated by several hundred times the dispersal distance (i.e.\ $R/\sigma > 308$).
The situation does not change much if the influx is quite a bit higher -- even if $A \mu = 1$,
the required separation between patches is only reduced to $R/\sigma > 9.5$ with $s_m=.05$ and $R/\sigma>154$ for $s_m=.001$.
...
\begin{equation} \label{eqn:total_migrate}
  \sum_{i=1}^k \migrate(R_i) = C \frac{ \rho s_p }{\xi^2 s_m} \sum_{i=1}^{k} \left(R_i \sqrt{2 s_m} /\sigma \right)^{-(d-1)/2} \exp\left(- R_i \sqrt{2 s_m} /\sigma\right).
\end{equation}
...
that is killed at rate $s_m$ \citep[see][2.2.0.1 and 4.2.0.1]{borodin2002handbook}.
...
In addition, over long time-scales, very rare chance events \citep[mice carried off by hurricanes and the like][]{censky1998overwater,nathan2008mechanisms}
