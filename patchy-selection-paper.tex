...
then by Bayes' rule, $(1-s_m)^t = (1-k_e(t))\E[K_t]$.
...
and that $(1-s_m)^{t}/(1-k_e(t)) \to \E[K]$ as $t \to \infty$.
...
i.e.\
\begin{align}
    u(x,y) = \int_0^t (1-s_m)^t p_t(x,y) dt .
\end{align}
...
\begin{align} \label{eqn:occupation_integral}
    q(y) &= \int_{\partial B(r_0)} \gamma(x) u(x,y) dx  . % \\
         % &= \int_{\partial B(r_0)} \gamma(x) \int_0^t (1-s_m)^t p_t(x,y) dt dx .
\end{align}
...
and that $(1-k_e(t)) \approx (1-s_m)^t/\E[K]$,
...
% \end{align}
Combining these terms with the expression for $g(A)$ from Appendix~\ref{apx:hitting_occupation},
\begin{align}
  \label{eqn:migrate}
  \migrate(y) \approx q(y) \times \frac{ \rho s_p  }{ \xi^2 \log(1/(1-s_m)) } 
...
\plr{want to do that? or be a bit more specific? what works/doesn't?}
...
whose chance of surviving until $t$ generations is $k_e(t) \simeq e^{-\alpha t} / \E[K]$.
