...
\title{
Convergent Adaptation while adapting 
}
...
or even by precisely the same genetic changes \citep{zhen2012parallel,,martin2013repeated,deephomologypapers}. 
...
under which a habitat configuration will support a stable polymorphic equilibrium \citep{cantrell1991diffusive},
...
haplotype of genetic length $1/(r_{eff}T)$ Morgans to remain linked to
the $B$ for a typical lineage. Note that the length of haplotype
...
suggesting these populations have adapted in parallel \citep{nachman2003different}.
...
Using $\sigma \approx 1km$ \gc{REF}, these cline widths imply $s_m=1/\sqrt{3}$ and
...
Finally, we set $A=100\text{km}^2$ (roughly thr size of the Pinacate patch).
...
either a new mutation or migrant allele to be large. We can use eqn
\eqref{eqn:haplotype_length} to calculate how long a founding
haplotype we initially expect to find shared between populations this in Figure \ref{fig:mice_prob_parallel}. The initial length can be
...
quite short. Furthermore, given the relatively small radius of the dark substrate
patches (often $<10km$) we should expect the haplotype to be rapidly
whittled down within patches after establishment (eqn. \ref{})
...
keeping ecological similar populations evolving in concert \citep{Sexton:14}.
