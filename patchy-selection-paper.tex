...
(e.g.\ serpentine outcrops and mine tailings) \citep{macnair1991evolution,schat1996identical,turner2010serpentine,mimulus};
...
% If the recombination rate per base is $r_{BP}$, 
% then the length of the haplotype associated with the $B$ allele is
% \begin{equation}
...
\plr{He referred to \citet{french1968dispersal,allred1963range}.}
...
\subsection{PETER'S VERSION}

This paper is an investigation into the basic, ill-formed question: 
What is the spatial resolution of adaptation?
In other words, what scale of environmental patchiness can a species develop and sustain local adaptations,
and over what spatial scale will evolution develop independent solutions to evolutionary problems?
The answer to this depends most strongly on $\sigma/\sqrt{s_m}$, 
the dispersal distance divided by the square root of the selection against the new allele between patches,
and much more weakly on the selective benefit within the patches or (perhaps suprisingly) the population density.
Weaker selection against alleles will make sharing of adaptations between patches easier,
but will also make it harder to spot,
since the lucky, transiting alleles move slower and carry a shorter haplotype.

Observations of similar adaptations occuring within the same species in widely separated patches
is strongly suggestive of parallel, independent adaptations.
Such observations are very interesting 
as they tell us about the degree of constraint faced by evolution --
how many different methods are there typically available to a species
with which to solve an evolutionary problem?

It is already well-appreciated that larger populations can lead to more parallelism \citep{softsweepsI}
\plr{fisher probably said this too?}
and that spatial structure contributes to this \citep{ralph2010parallel}.
Naturally, then, local adaptation to well-separated patches of some environment is even more likely to occur in parallel;
we have studied the case where patches are separated by long distances.
This is complementary to the work of \citet{XXX}, who studied local adaptation in rapidly varying environments.
Intermediate cases where habitat types are distributed in some more complex geometry
-- archipelagos of patches of varying sizes, or connected by long, skinny corridors, for instance --
remains to be studied.

To complement this theory, we need more empirical work,
in which the parameters (migration rate, sensitivity to environmental factors, etc)
are reasonably well-understood.
However, learning about these aspects is difficult in the field,
and it is still not understood how much of the intraspecific variation we observe
is due to adaptation to local conditions, or even how to tell.
Notable steps forward include \citet{turner2010serpentine}, Mimulus, Amity, others?
...
On the other hand, a dominant mutation would not differ greatly from the theory presented here.
In fact, MC1r is ?dominant? and agouti is ?recessive?
Other schemes could be obtained.


optimal $s_m$ for migration makes migration harder to spot

\subsection{GRAHAM'S VERSION}

We have developed a simple set of approximations to describe the rate
at which organisms in a patchy environment adapt through novel
mutations or migration from other nearby patches (eqn.
\eqref{eqn:mutrate} and \eqref{eqn:migrate}), and the relative
probability of each of these outcomes (eqn. \eqref{eqn:parallel_prob}). 
We have also determined the characteristic genomic length scales over which
the various signals of hitchhiking should be visable
(eqn. \eqref{eqn:haplotype_length} and ). 

These results show that the probability of parallel adaptation is
independent of both the population density and the beneficial
selection coefficient. This follows from the fact that these
parameters both increase the rate of adaptation via both migration and
mutation equally. Thus they are important for the time-scale over
which adaptation occurs but not the mode of adaptation. This
constrasts with models of convergent adaptation within panmictic
populations \citep{} and geographically spread populations adapting to
homogeneous populations \citep{RalphCoop}, where the number of
adaptive alleles increases with the population size/density. In all of these models the dependence on the beneficial selection
coefficient is absent or weak, due to the fact that selection both
aids establiishment of new alleles and the spread of existing
alleles. 

The primary determinant of the probability of parallel adaptation is
$\sigma/\sqrt{s_d}$, the characteristic geographic distance of the
allele frequency decay off the patch. We find that patches that are close together on
this scale will adapt via migration, but there will be a sharp
transition to convergent adaptation as we move to patches that are further apart. 
 
In coming to this conclusion, we have ignored the possibility of very long distance migration,
...
We have also assumed that the alleles have a fitness effect in
heterozygotes both on and off the patch. However, 
 recessive alleles could also contribute to adaptation to patchy
 environments. \gc{should we cite Nachman eg here}.  Our
 simple branching process framework does not easily accomodate recessive
 alleles as it ignores the selection interaction of alleles, and so a
 different approach will be needed. At first sight it
 seems like recessive alleles may be able to contribute to adaptation
 via migration over longer distances, as the migrant alleles will be at low
 frequency between patches and so usually invisible to
 selection. However, this fact also means that they are unlikely to
 establish on the beneficial patch upon reaching it via migration. As
 such it is not clear what our intuition should be about the
 contribution of recessive alleles to adaptation via migration. 
  
\gc{will write a couple more broader paragraphs}


The falling cost of sequencing over the next decade offers the
opportunity to investigate the genomics of parallel adaptation over a
large s
Over the coming decade population genomic data will be generated  
...
It should be possible to argue that if $R$ is large then the value of $s_*$ won't make much difference.
