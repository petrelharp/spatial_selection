...
% OR NOT \bibliographystyle{plos2009}
...
% Simon: I think the title only partly reflects the scope of the paper (adaptation by migration being underrepresented). What about "Relative importance of migration and convergent evolution for local adaptation in patchy landscapes" or similar?
...
The most important conceptual work of the paper occurs in section \ref{ss:patchymigration},
...
We combine these two results in section \ref{ss:probparallel} 
...
in section \ref{ss:haplotype_length}, 
...
(equation \eqref{eqn:within_haplotype_length}), in section \ref{ss:patch_haplotype}. 
...
(Here we take ``fitness'' to be the intrinsic growth rate of the type when rare.)
...
Therefore, we do not have to specify the fitness of the homozygote; 
...
We examine this more generally via exact calculation of $p(x)$ in Section \ref{apx:establishment_sims}.
...
along the way obtaining other useful information about the genealogy of $B$ alleles.
...
the effective rate of at which \emph{families of} migrant $B$ alleles establish in a new patch at displacement $x$ from the original,
is
\begin{align}
...
the distance between patches must be at least 30 times the dispersal
distance (i.e.\ $R/\sigma > 30$) for convergence to occur.
...
that is killed at rate $s_m$ -- \citet[see][(2.2.0.1 and 4.2.0.1)]{borodin2002handbook}.
...
and to convert it to a speed this is divided by $1/s_m$.
...
if the assumption that fluctuations in $p(x)$ are small,
...
% multiplied by $p(x)$ in these intermediate regions, where we assume $p(x) \approx 1/2$. 
...
and hence the genetic length of the haplotype shared between pairs of
...
alleles \citep[but see][for the complications of varying population sizes]{Wilson-softsweep}. 
...
is $R = (\sigma/\sqrt{2s_m}) \log(1/(s_m A \mu))$.
...
\paragraph{Other patch geometries}
...
may be forced to locally adapt convergently when migration is geographically limited
...
We would like to acknowledge Michael Nachman, Steve Evans, and Yaniv Brandvain for useful discussions, 
as well as Gideon Bradburd and the rest of the Coop lab for helpful comments on the manuscript.
