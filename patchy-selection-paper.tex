...
Suppose the new patch has width $w$, and is at distance $x$ from the original patch.
If the new patch is small ($w \ll \sigma/\sqrt{2s_m}$), 
then $q(S)$ is approximately $q(x)$ multiplied by the area of the new patch.
If the new patch is a long rectangle with width $u$ with long axis pointed towards the old patch,
$q(S)$ is approximately $u C \sqrt{\sigma/\sqrt{2s_m}} \erf(\sqrt{x\sqrt{2s_m}/\sigma})$,
