...
This change could be as specific as a single base change, a knock out
of a gene, or a mutation anywhere in a set of genes. We assume that
this mutational change occurs at a (low) rate of $\mu$ per chromosome per generation. 
...
Outside of these patches we assume that $B$ has a relative fitness
$1-s_m$  when rare in the intervening areas, with $s_p, s_m > 0$.
...
migration in spatially varying selection models. \gc{cite Haldane and zfisher} 
Haldane, Fisher, and later \citet{slatkin1973geneflow} first analyzed
...
Let $p(x)$ be the probability that a new mutant $B$ allele arising at
location $x$
...
but these are not particularly helpful.
The total successful mutational influx per generation is given by the
...
If we consider a patch of area $A$, whose width of the patch is large relative to $\sigma/\sqrt{2s_m}$, 
...
\gc{I'd suggest making Figure 2 and Fig 1A all part of same figure
  they are showing the same basic thing good fit of equation of
  cline. The s's in 1A should be labeled sm and sb. I'd suggest that
  the second part of 1 become its own figure or be paired with fig.3
  as they are both about rare excursions. Could you make the CI or
  sample path in Figure 1B be different colors, the grey and black are
...
More general, asymptotics of the solution implies that for large $|x|$ the mean occupation frequency decays exponentially, 
...
Therefore, if we look at a small area of size $dx$, 
then $q(r) dx$ is equal to the probability of finding any $B$ alleles there,
...
the equilibrium is composed of rare excursions of a family of $B$
alleles, interspersed by long periods of absence. An example of one of these rare
excursions of a migrant families is shown in Figure \gc{XXX} in \gc{XXXcolour}.
...
\gc{We now work through the steps in developing this approximation,
  and give our resulting approximation for $\migrate(y)$ in eqn. \eqref{eqn:migrate}. } 
...
Therefore, if we define $p_e(t)$ to be the chance of a \gc{family}
going extinct by time $t$, 
...
\gc{We exploit this idea to consider the survival of a central ``trunk'',
  and then conditional on that consider the family size of side
  ``branches'' off of this trunk. This process is depicted in Figure~\ref{fig:branching_decomp}.}
...
at least one lineage surviving from time $0$ to time $t$ (the long red
lineage in Figure \ref{fig:branching_decomp}).
\gc{Given this lineage, nothing distinguishes any of the other reproduction events --
each individual along this lineage} must give birth to at least one offspring,
...
\end{align}
where each $Z^{(s,k)}$ is an independent copy of $Z$ \gc{that arose
  from the main branch in generation $s$},
and each $W_s$ is an independent copy of the offspring distribution, conditioned on $W_s \ge 1$.
\gc{Is there no 1+ in the equation above for the main branch? }
...
Figure~\ref{fig:sim_snapshots} has a depiction of such a family, from
a simulation. \gc{In reality the spatial paths taken by lineages are
  much more wiggly but they are shown as straight lines here for simplicity.}
...
it will turn out that eqn. \eqref{eqn:gestalt_migrate} is a constant multiple of \eqref{eqn:gestalt_q}.

First consider eqn. \eqref{eqn:gestalt_q}.
Suppose that $Z$ is one such spatial branching process as above, started with a single individual at $x$,
defined so that $Z_t(A)$ is the number of descendant individuals alive at time $t$ living in the \gc{area} $A$.
...
\end{align}
\gc{$\alpha$ introduced here? Switch from $(1-s_m)^t$ }
...
Now we turn to the expression \eqref{eqn:gestalt_migrate} for the outflux of successful migrants.
...
% The probability that this family reaches $A$ at time $t$ is the probability that it survives until $t$, $(1-p_e(t))$,
% multiplied by the probability that the family reaches the patch, given survival.
% If $t$ is large then as in the previous section, $(1-p_e(t)) \approx e^{-\alpha t}/\E[K]$.
It would be possible to analyze directly the probability that a given migrant family establishes in $A$,
as in \citet{barton1987establishment,mckean1975KPP};
but we make a simpler approximation, approximating this by the chance that the ``trunk'' hits the new patch,
multiplied by the chance that at least member of the family escapes demographic stochasticity to establish.
...
As for $q(y)$ above, expression \eqref{eqn:gestalt_migrate} is an integral of $h(x,y)$:
...
Using this fact,
and that $(1-p_e(t)) \approx e^{-\alpha t}/\E[K]$,
it turns out (see Appendix~\ref{apx:hitting_occupation}) that
\begin{align} 
  h(x,y) = \frac{ s(A) }{ \E[K] } u(x,y),
\end{align}
where $s(A)$ is the total expected occupation time of the patch given the family arrives,
...
(at least asymptotically for large distances),
we now know that the outflux of successful migrant families is equal to the mean occupation frequency
multiplied by $s(A) f(A) / \E[K]$.
The probability $f(A)$ that the family does not die out once it arrives at the new patch
is if each member arriving has chance $p_e$ of establishing, independently, $\E[1-(1-p_e)^K] \approx 2 s_p \E[K] / \xi^2$.
...
% \end{align}
Combining these terms with the expression for $s(A)$ from Appendix~\ref{apx:hitting_occupation},
\begin{align}
  \label{eqn:migrate}
  \migrate(y) \approx q(y) \times \rho \frac{ s_p  }{ \alpha \xi^2 } 
  % \left( 1 - \left( \frac{\pi w \sqrt{2 \alpha}}{ 2 \sigma } \right)^{\frac{d-1}{2}} e^{-w \sqrt{2\alpha}/\sigma} \right).
\end{align}
For patches that are small relative to $\sigma/\sqrt{2s_m}$, there is an additional correction term (see Appendix~\ref{apx:hitting_occupation}),
but small patches violate other assumptions above, so we omit it.
...
and will only slowly mix together over longer time scales by drift \gc{and
migration. We made use of this selective exclusion of alleles in our
previous paper \citep{RalphCoop}. In reality different mutations may
not be truly selective equivalent in which case the mixing will occur
over a time-scale dictated by the difference in selection
coefficients. } 

We will assume that once a $B$ allele is introduced (by migration or mutation), 
\gc{condition on it escaping loss,} it becomes established in the poor habitat patch rapidly. 
...
simply comes down to whether a new mutation or a migrant allele is the first to become established in the second patch. Taking expressions \eqref{eqn:mutrate} and \eqref{eqn:migrate} above,
...
and recall that $A$ is the area of the not-yet-adapted patch.
...
Equivalently, migration will occur much quicker than mutation 
if $R/\sigma > (\log(1/s_m) - \log(A\mu))/\sqrt{2s_m}$.
\gc{the two paragraphs either side of this will need some reworking.}

If we take the mutational influx in a patch to be one per 1,000
generations ($A \mu = .001$), \gc{don't think this is mutational
  influx as it doesn't have $\rho in it$}
and the selection against transiting alleles $s_m = .01$,
adaptation procedes independently for distances $R/\sigma > \log(s_m)/\sqrt{s_m}$,
so if the selective pressure is fairly strong (say, $s_m=.05$),
then the distance between patches must be about 30 times the dispersal distance (i.e.\ $R/\sigma > 30$),
...
The situation does not change much if the influx is quite a bit higher -- even if $A \mu = 1$,
the required separation between patches is only reduced by $7/\sqrt{s_m}$,
to $R/\sigma > 10$ with $s_m=.05$ and $R/\sigma>150$ for $s_m=.001$.
If the separation between patches is much larger than this transitional value -- certainly, by a factor of ten -- 
...
as a function of various parameters. \gc{Parallel mutation will occur
where $\Tmut \gg \Tmig $ (reder areas), while adpatation by migration will occur
where $\Tmig \gg \Tmut $ (bluer areas).}
...
\begin{figure}[ht]
  ...
  The line at which $\Tmig=\Tmut$ is shown in black.
...
Both these times scale linearly with $\xi^2/(\rho s_{B})$, 
...
\begin{equation}
  \frac{\mutrate}{\mutrate+\migrate(r)} = \frac{ A \mu \alpha }{2 A \mu \alpha +
    \left(r \sqrt{2 s_m} r / \sigma \right)^{(d-1)/2} \exp\left(- \sqrt{2 s_m} r / \sigma \right) },
\end{equation}
...
If not yet adapted, a patch of area $A$ will acquire the adaptation by new mutation at rate $2 s_p \mu \rho A/\xi^2$.
...
then the total rate of adaptation through migration is approximately
\begin{equation}
  \frac{ s_p \rho }{\alpha \xi^2} \sum_{i=1}^{k} \left(r_i \sqrt{2 s_m} /\sigma \right) \exp\left(- r_i \sqrt{2 s_m} /\sigma\right),
\end{equation}
and conditional on coming from migration the probability the adapted allele 
comes from patch $i$ is 
\begin{align}
  \frac{\left(r_i \sqrt{2 s_m} /\sigma \right) \exp\left(- r_i \sqrt{2 s_m}
    /\sigma\right)} {\sum_{j=1}^{k}  \left(r_i \sqrt{2 s_m} /\sigma \right) \exp\left(- r_i \sqrt{2 s_m}
    /\sigma\right) } .
\end{align}
...
share a fairly long stretch of haplotype. \gc{Pairs of individuals
  within a patch will share a haplotype of mean genetic length of
  $\approx \approx \log(\rho A s_p)/s_p$ around the selected site,
as long as the patch is reasonably well mixed by dispersal
\citep[otherwise see][]{BartonEtheridge13spatialHH}. }
...
whose chance of surviving until $t$ generations is $p_e(t) \simeq e^{-\alpha t} / \E[K]$.
Consider a single such family, and let $\tau$ be the (random) time it first hits the new patch,
with $\tau = \infty$ if it never does. The length of the hitchhiking segment depends on the time
spent by this trunk lineage ``in transit'' between the patches, i.e. $\tau$.

By pulling out the constant $1/\E[K]$, we can use standard results on hitting times of $d$-dimensional Brownian motion
that is killed at rate $\alpha$ \citet[][(2.2.0.1 and 4.2.0.1)]{borodin2002handbook} 
In particular, if the patch is circular with radius $w$ and a distance
$R$ from the already adapted patch, then we have the useful result
about the Laplace transform of $\tau$ our transit time
\begin{align}
  \E[e^{-\ell \tau}] &=
    \begin{cases}
      e^{- R \sqrt{2(\alpha+\ell)}/\sigma} \qquad & d=1 \\
      \frac{ K_0( (R+w)\sqrt{2(\alpha+\ell)}/\sigma) }{ K_0( w\sqrt{2(\alpha+\ell)}/\sigma) } \qquad & d=2 
    \end{cases} \label{borodinresult}
    % &= \left( R/w +1 \right)^{-\frac{d-1}{2}} e^{- R \sqrt{2(\alpha+\ell)}/\sigma}  (1+O(1/w)) .
\end{align}
\gc{define $K_0$, bessel? We've used K for family size}
We can find that the probability that a particular
trunk lineageis killed before it reaches the new patch is
$\P\{\tau < \infty\} = \lim_{\ell \to 0} \E \left[\exp(-\ell \tau)
\right]$. We are interested in lineages that
manage to establish on the patch i.e. $\tau< \infty$, and so in one dimension
\begin{equation} \label{eqn:haplen_cdf}
\E[e^{-\ell \tau}|\tau<\infty]  = \exp\left\{{-\frac{R}{\sigma}\left(\sqrt{2(\ell+\alpha)} - \sqrt{2\alpha}\right)}\right\} .
\end{equation}
We can differentiate this to find that our expected transit time is
\begin{equation}
\E[\tau] = (R\sqrt{2\alpha}/\sigma)\times 1/(2\alpha)
\end{equation}
and $\var[\tau] = (R\sqrt{2\alpha}/\sigma) \times 1/(2\alpha)^2$ in
one dimension. 

% \simeq (R/w+1)^{-(d-1)/2} \exp(- R \sqrt{2\alpha}/\sigma)$.
...
initially shared between populations on each side of the site will be exponentially distributed
...
\end{equation}
which is our Laplace transform from above that in one dimension
gives us eqn. \eqref{eqn:haplen_cdf}. This implies that in one dimension, if $Y$ is an exponential random variable with rate $R\sqrt{2}/\sigma$,
then $L$ has the same distribution as $(Y + \sqrt{\alpha})^2 - \alpha$,
from which we can obtain that the expected length of shared hitchiking
haplotype is
\begin{equation} \label{eqn:haplotype_length}
\E[L] = \sigma \sqrt{2\alpha}/R + \sigma^2/R^2
\end{equation}
and thar $\var[L] = 2\alpha\sigma^2/R^2 + 4 \sigma^3 \sqrt{2\alpha}/R^3 + 5 \sigma^4 / R^4$.
For two dimensions, asymptotics of Bessel functions show that $L$ has the same tail behavior,
but how other properties might change is unclear. \gc{add some
  sentences of intuition of how parameters play into haplotype length
  here. is it really $\sigma \sqrt{\alpha}$ in \eqref{eqn:haplotype_length} and not $\sqrt{\alpha}/sigma$???}

\gc{Perhaps we should separate the above bit into things that are
  useful about $\tau$ and put those up by  \eqref{borodinresult} and
  things that are useful about $L$ here? We should also turn some of
  this into numbered equations. I tried to do this above, and have the
commented out original below}
...
% with mean $\tau$, so that if $L$ is the length of hitchhiking segment on, say, the right of
% the selected locus, then in one dimension
% \begin{align} \label{eqn:haplen_cdf}
% \P\{L>\ell\} &= \E[e^{-\ell \tau}|\tau<\infty] \\
%   &= \exp\left\{{-\frac{R}{\sigma}\left(\sqrt{2(\ell+\alpha)} - \sqrt{2\alpha}\right)}\right\} .
% \end{align}
% found by using eqn. \eqref{borodinresult}. This is also the Laplace transform of $\tau$;
% we can differentiate it to find that $\E[\tau] = (R\sqrt{2\alpha}/\sigma)\times 1/(2\alpha)$
% and $\var[\tau] = (R\sqrt{2\alpha}/\sigma) \times 1/(2\alpha)^2$.
...
so that $\tau = R/\sigma(2\alpha)^{-1/2} + \eta_\tau \sqrt{R/\sigma} (2\alpha)^{-3/4}$.
...
XXX Check what happens in two dimensions. XXX
...
\begin{align}
    F(x,y) &= \int_0^\infty (1-p_e(t)) \P\{ \tau_A \in dt \} \\
           &= \int_0^\infty e^{\alpha t} (1-p_e(t)) e^{- \alpha t } \P\{ \tau_A < t \} dt \\
    &=\int_0^\infty e^{\alpha t} (1-p_e(t)) \P\{ \tau_A < t < \tau_\dagger \} dt \\
...
\begin{align}
    q(y)  &= \int_{\partial B(r_0)} \gamma(x) u(x,y) dx  \\
          &= h(A) \int_{\partial B(r_0)} \gamma(x) F(x,y) dx \\
          &= \frac{ h(A) }{ \E[K] } \migrate(y) .
\end{align}
The function $h(A)$ is the expected amount of time that a Brownian motion begun on the edge of a disk of area $A$
