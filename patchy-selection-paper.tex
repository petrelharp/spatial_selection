...
we have that the effective outflux (at the edge of the already occupied patch!) is 
\begin{align}
  (\mbox{outflux of BPs from old patch}) = C \rho s_m ,
\end{align}
...
\begin{align}
  ...
  &= C \rho s_m \times \frac{2 s_b}{\xi^2} \exp\left(- \frac{\sqrt{\alpha}x}{\sigma} \right) .
\end{align}
Finally, combining these, we can also compute that
\begin{align}
  (\mbox{\# estab events per successful BP}) = \frac{s_B+s_m}{s_m} .
\end{align}
\plr{end of notes}
