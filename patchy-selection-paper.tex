...
\begin{align} \label{eqn:eqfreq}
    q(x) \approx C (|x|-r_0)^{-(d-1)/2} \exp( - |x| \sqrt{2 s_m} / \sigma) \qquad \text{for large $|x|$},
\end{align}
...
From simulations, $C$ seems to be close to 1 in practice (see Appendix \ref{apx:patchy_sims})
and XXX $r_0$.
...
\begin{align}
    u(x_0,x) &= \frac{1}{\sigma^d \pi}\left(\sqrt{ \frac{\|x-x_0\|^2}{2\alpha\sigma^2} }\right)^{(d+2)/2} K_{(d+2)/2}\left( \sqrt{2\alpha} \frac{\|x-x_0\|}{\sigma} \right) \\
             &= \frac{1}{\sigma^d \sqrt{2\pi}} \left(\sqrt{ \frac{\|x-x_0\|^2}{2\alpha\sigma^2} }\right)^{d/2} \exp\left(- \sqrt{2\alpha} \frac{\|x-x_0\|}{\sigma} \right)+ O(\sigma/\|x-x_0\|) . \label{eqn:occupation_asymptotics}
\end{align}
...
However, this integral is not tractable; if we assume that $r_0$ is small relative to $\|x\|$, then we get 
\begin{align}
    q(x) &= \int_{B(r_0)} u(y,x) \gamma(|y|) dy \\
         &\simeq \gamma(r_0) 2 (\pi r_0)^{d-1} u(0,x) .
\end{align}
...
using \citet{gradshteyn2007table} 8.494.8,
the general solution is
\begin{align}
    u(r) = C' (r-r_0)^{d/2} K_{d/2} \left( (r-r_0) \sqrt{2s}/\sigma \right) ,
\end{align}
...
\begin{align}
    q(r) \simeq C (r-r_0)^{(d-1)/2} \exp \left( -r \sqrt{2s}/\sigma \right) ,
\end{align}
