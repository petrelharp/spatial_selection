...
If the influx is higher -- say, $\rho A \mu = .01$,
the required separation between patches is only reduced to $R/\sigma > 7.3$ with $s_m=.05$.
If $\rho A \mu = .01$ and $s_m=.001$, then the prediction is that mutation will \emph{always} be faster than migration
-- however, with $s_m=.001$, for the assumption of well-separated patches to hold we would need $R/\sigma > 1/\sqrt{2s_m} = 22.4$;
therefore, at spatial scales smaller than about 20 dispersal distances the model breaks down.
...
\begin{figure}[ht]
  ...
  The right-hand plot shows that $\migrate$ is relatively insensitive to $s_m$.
