...
and each $W_s$ is independent with distribution $\P\{ W_s = k\} = \P\{ W = k \} (1-k_e(t-s)^k)/(1-k_e(t-s+1))$, 
...
\begin{align} \label{eqn:mean_K}
    \E[K] 
    % = 1 + \E[W-1 \given W \ge 1] /(1-e^{-s_m}) 
    = 1 + \sum_{s=0}^\infty \E[W_s-1] e^{-s_m t}
    \approx \E[W-1 \given W \ge 1] / s_m .
\end{align}
...
Combining these, and absorbing the term $\E[W-1|W>1]$ from equation \eqref{eqn:mean_K} 
into the constant $C$,
...
\begin{figure}[ht!]
  ...
  \caption{
      ...
      \ref{sfig:sim_migration_times} and \ref{sfig:sim_mutation_times}.
  }   \label{fig:sim_times}
\end{figure}
...
\begin{align}
    q(S) \le |S| q(x) % C \left( \frac{x\sqrt{2s_m}}{\sigma} \right)^{(d-1)/2} e^{-x\sqrt{2s_m}/\sigma} ,
\end{align}
