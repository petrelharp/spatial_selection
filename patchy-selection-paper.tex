...
\textbf{(b)} the inter-patch distance is at least the characteristic length
($R > \sigma/\sqrt{s_m}$),
and that
...
but don't know a good way to estimate this contribution.
...
\begin{equation} \label{eqn:parallel_prob}
    \frac{\mutrate}{\mutrate+\migrate(R)} = \frac{ A \mu / (2s_m) }{ A \mu / (2s_m) + \gamma \left(R \sqrt{2 s_m} /\sigma \right)^{-(d-1)/2}\exp\left(- \sqrt{2 s_m} R / \sigma \right) },  
\end{equation}
