...
\citet{slatkin1973geneflow}, working in a one dimensional, deterministic framework, 
showed that if the physical width of the patch is less than $2 \tan^{-1} (\sqrt{s_m/s_p})$, 
...
As shown in Appendix \ref{apx:elliptic_integrals},
...
sprouting independent ``branches'' that may or may not survive until $t$.
...
or \citet{etheridge} or \citet{dawson} for general theory used below.
...
Suppose that $X$ has mean $\mu$; the variance of $X$ will not enter into the formula.
Also suppose that $R$ has mean zero and variance $\sigma^2$;
...
First, since this is a first-order differential operator, the limiting stochastic process
...
leading to the same asymptotic behavior (CITE SOMETHING).
...
Or, in other words, solving the Fisher-KPP equation with varying selection.
