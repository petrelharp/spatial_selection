...
\gc{add some
  ...
  here. is it really $\sigma \sqrt{s_m}$ in \eqref{eqn:haplotype_length} and not $\sqrt{s_m}/sigma$???}

\gc{Perhaps we should separate the above bit into things that are
  useful about $\tau$ and put those up by  \eqref{eqn:borodinresult} and
...
This means that in time $dt$, a total of $ dt \, \rho p(y) (1+s(y)) m(y,x) \, dy \, dx $ new, type B migrants 
will move from an area near $y$ of size $dy$ to an area near $x$ of size $dx$.
Looking backwards in time, this is the number of $B$ alleles living at $x$
who were produced by a parent from near $y$ in time $dt$.
...
Doing this for the Tule we obtain $\sigma/\sqrt{s_m} \approx 3$km and for the O'Neill site
$\sigma/\sqrt{s_m} \approx 30$km, giving us a range of cline widths. We also need estimate of $s_m$,
using  $\sigma=1km$ \gc{REF} our cline widths imply $s_m=3^{-1/2}$ and
$30^{-1/2}$ are reasonable values.
...
broadly across vertebrates \citep{REF}.To span a range of plausible
values we use a low mutation rate of $\mu= 10^{-8}$, where the adaptive
mtation has to occur at a single base pair; and a high mutation
rate $\mu= 10^{-5}$, equivalent to the mutation being able to arise at any of a thousand base pairs
e.g. any of the coding bases of a typical length gene.  We set $A=100km^2$ reflecting a patch with a width of $\sim 10km$. 
