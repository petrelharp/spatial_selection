...
To really make use of the expressions in the previous section,
...
To do this, we compare the rate of recombination between B and b haplotypes
to the rate of coalescence of B haplotypes within a patch,
which we take to be $1/(A\rho)$ 
(bypassing the well-known difficulties to spatial coalescence \citep{felsenstein1978pain,bartonetheridgedepaulis}).

Consider the lineage of a single B allele, moving back through time,
in a patch at mutation-selection balance, 
i.e.\ with the local proportion of B alleles near $x$ equal to $p(x)$,
and that the lineage stays close enough to the patch 
that fluctuations about $p(x)$ are small (unlike in previous sections).
...
This is a diffusion that, when at $x$, 
...
Or, more simply, if recombination is on a slower time scale than mixing within the patch,
...
so recombination between backgrounds is unlikely to occur,
...
is roughly $2(\sigma/\sqrt{s})A^{-1/d}$,
and so the rate of recombination of a linked lineage to the b background
is, taking $p(x) \approx 1/2$ in this region, roughly $r (\sigma/\sqrt{s})A^{-1/d}$,
