...
below provide a derivation of \eqref{eqn:eqfreq} in section \secref{apx:eqfreq}, 
...
\begin{align} \label{eqn:mean_K}
    \E[K] 
    % = 1 + \E[W-1 \given W \ge 1] \lim_{t \to \infty} \sum_{u=0}^t e^{-s_m u} = 1 + \E[W-1 \given W \ge 1] \lim_{t \to \infty} \frac{1-e^{-s_m (t+1)}}{1-e^{-s_m}}
...
In one dimension, we are done (and discuss exact solutions in Appendix~\ref{apx:elliptic_integrals});
...
i.e.\ the time-averaged proportion of individuals near a point $x$ in $\partial B(r_0)$ 
...
However, examining the constituent terms,
...
as the expected total number of offspring of a family beginning at $x$ that ever live near $y$.
...
because both $q(x)$ and $\gamma(x)$ are local \emph{proportions} of $B$ alleles.
...
and using $(1-k_e(t)) \approx e^{-s_m t}/\E[K]$,
we show below that
\begin{align} 
...
(Before, this was $h_A(x,y)$, here modified to parallel $u(x,S)$.)
...
Let $\xi(t,x)$ be the proportion of the allele under spatially varying selection $s(x)$ in one dimension,
so that 
\[
