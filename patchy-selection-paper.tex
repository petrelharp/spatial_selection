...
The total successful mutational influx per generation is given by the integral of $p(x)$ over the entire species range,
% $\int \rho \mu p(x) dx$,
...
it will turn out that \eqref{eqn:gestalt_migrate} is a constant multiple of \eqref{eqn:gestalt_q}.

First consider \eqref{eqn:gestalt_q}.
...
If we take the mutational influx in a patch to be one per 1,000 generations ($A \mu = .001$),
...
\begin{equation}
  \frac{\mutrate}{\mutrate+\migrate} = \frac{2 A \mu s_m}{2 A \mu s_m + \exp\left(- \sqrt{2 s_m} r / \sigma \right) },
\end{equation}
