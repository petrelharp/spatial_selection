...
\begin{equation}
  \frac{ s_p \rho }{s_m \xi^2} \sum_{i=1}^{k} \exp\left(- r_i \sqrt{2 s_m} /\sigma\right),
\end{equation}
and the probability that the migrant, adapted allele 
comes from patch $i$ is 
\begin{align}
  \exp\left(- r_i \sqrt{2 s_m} /\sigma\right)/\sum_{j=1}^{k} \exp\left(- r_j \sqrt{2 s_m} /\sigma\right) .
\end{align}
...
that is killed at rate $\alpha$.
In particular, we know that if the patch is circular with radius $w$, then
using \citet[][(2.2.0.1 and 4.2.0.1)]{borodin2002handbook}
\begin{align}
  \E[e^{-\ell \tau}] &=
    \begin{cases}
...
$\P\{\tau < \infty\} = \lim_{\ell \to 0} \E[\exp(-\ell \tau)]$. % \simeq (R/w+1)^{-(d-1)/2} \exp(- R \sqrt{2\alpha}/\sigma)$.
...
with mean $\tau$, so that if $L$ is the length on, say, the right, then in one dimension
