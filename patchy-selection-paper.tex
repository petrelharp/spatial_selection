...
\gc{Graham pasted this to the appendix as it felt like too much information to drop on people}
\plr{I am sympathetic, but it's only a paragraph\dots how about that last sentence, instad?}
%%%
% 
...
\begin{align}
  r_\text{eff} &=
    \begin{cases}
      r \frac{2\sigma}{A\sqrt{s}}    \qquad & d=1 \\
      r \frac{\sqrt{\pi} \sigma }{ \sqrt{ s A } } \qquad & d=2 \\  
\end{cases} ,
\label{eqn:rate-rec-in-patch}   %(\sigma/\sqrt{s})A^{-1/d}
\end{align}
i.e.\ the reduction in recombination rate is proportional to $(\sigma/\sqrt{s})A^{-1/d}$.
