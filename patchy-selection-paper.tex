...
%%%%%%%%%%%
\abstract{
When the same phenotype is found to be responding to selection in parallel in isolated parts of a species range,
it provides strong evidence as to the degree of constraint imposed on evolution by the underlying genetic architecture.
Migration can counfound such inferences by synchronizing adaptation across the range;
but the importance of such sharing of local adaptations is unclear.
This paper studies the balance between these two in continuous environments with patchy selection pressures,
such as when an animal's coloration is selected to match its background,
but the species range contains large areas of differently colored substrate.
The main questions answered are: 
How long does it take for a new, locally adapted allele to appear in a patch of habitat through new mutation?
Or, through migration from another, already adapted patch?
Which is more likely to occur, as a function of patch separation?
How can we tell which has occurred?
These are answered by decomposing the migration--selection equilibrium
...
The speed at which successful migrant alleles traverse regions where they are maladapted
determines the length of the haplotype shared by around shared, adaptive alleles,
which is compared to the length of the haplotype shared within a patch.
Cryptic coloration in the rock pocket mouse, \textit{Chaetodipus intermedius}, is discussed as an empirical example.
...
\citep{stern2013genetic}. 
\plr{want to also cite \citep{zhen2012parallel,martin2013repeated}?}
...
speciation under some scenerios \citep[in a manner similar to ][]{Kondrashov:03}. 
