...
At first it is tempting to argue
that since the probability of establishment of a beneficial allele is $p_e$,
and in a patch of area $A$ there are $\rho A q(x)$ beneficial alleles present at equilibrium,
the mean rate of establishment is $p_e \rho A q(x)$.
This is wrong, since it ignores the family structure of migrants.

However, this wrong start suggests a correct one:
notice that the mean density $q(x)$ counts up each family's total size,
while $\migrate(x)$ counts the probability the family establishes.
We argue below that 
...
% \end{align}
Combining with the fact that $g(A)\approx 1/(2s_m)$ (see section~\nameref{apx:hitting_occupation}, in particular equation \eqref{eqn:q_migrate_relation}),
we obtain that
\begin{align}
  \label{eqn:migrate} 
