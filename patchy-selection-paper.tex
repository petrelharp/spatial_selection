...
\setcounter{secnumdepth}{0}
   ...
   \newif\iflatexml\latexmlfalse %%%graham added

%We should cite Papers by Slatkin and Wiehe and this, on spatial hitchhiking http://www.ncbi.nlm.nih.gov/pmc/articles/PMC3176130/
%%%%%
% Responses to reviews:
\input{review-response-commands}
% set this to show line numbers and include responses to reviews or not
\newif\ifreviewresponses\reviewresponsestrue  % include them
...
\gc{GENERAL GRAHAM comment, every place where we give a section name nameref
I've made sure that we say section nameref as otherwise it looks weird  }
...
are more important \citep[see e.g.][for discussion]{levin2003ecology,ralph2010parallel,hallatschek2014acceleration}.  \linelabel{rr:longtailed}
...
obtaining along the way other useful information about the genealogy
of $B$ alleles. \gc{Doing this we arrive at \eqref{eqn:migrate} for
  the rate at which patches adapt via migration. } %%Thought it good
                                %%to signpost again where we are
                                %%going. 
...
As J.\ Hermisson pointed out \gc{in comments} on an earlier draft,
...
\linelabel{rr:q_form}  \gc{OR WE could just personal ref Hermisson here.}
...
we provide a derivation below in section \nameref{apx:eqfreq}, 
as well as justification for the asymptotics below in section \nameref{apx:asymptotics}.
...
are encouraged to take a detour through section \nameref{ss:hitting_occupation}, below.)
...
\gc{Note that the approximation has the undesirable property that it is
...
i.e.\ the parameter region that we are interested in.}
\linelabel{rr:sm_condition}
%%GRAHAM felt like we needed to be clearer on this point.
...
%%Graham brokke this paragraph up.
\gc{To be more precise,} the mutation rate results will hold best if $s_m \approx s_p$,
...
run for 25,000 generations. \gc{We simulated $\rho$ diploid
  individuals per patch, with nearest neighbor migration and
  multiplicative selection??!? }
More details are given below in section \nameref{ss:simulations},
...
\gc{We first consider the order of magnitude that our parameters need to
be on in order for adaptation via mutation or migration to dominant.} Let $\gamma = \min(1,s_m/p_e)$.
...
\gc{We can go beyond these orders of magnitude calculations to find
  the probability of parallel adaptation if we are will to take our approximations at face value, 
and assuming both rates are small. Doing so } 
...
approximately logistically with the distance between the patches, on the spatial scale $\sigma/\sqrt{s_m}$. \revpoint{2}{13}
...
Pairs of adapted individuals within a patch will \gc{initially} share a haplotype of mean genetic length of
...
\gc{In conclusion, the length of shared haplotype drops off quickly with the
distance between the patches. When $s_m$ is small we expect shorter
haplotypes as the allele is only weakly deleterious and so 
traverses the distance between patches relatively slowly compared to
strongly deleterious alleles. Therefore, when adaptation by migration
is most likely (small $s_m$) 
we expect shorter
shared haplotypes.}


%GRAHAM commented out this This intuitive picture can be expanded by a central limit theorem:
...
thumb $1$cM is approximately $1$Mb in a number of mammalian species. 
...
\gc{We note that questions about the genomic length-scale of signals of
sweeps shared by migration have also been addressed in discrete
population settings \citep{REFSCOMMENTEDBELOW}. This work has shown
that the length of shared sweept haplotype is often significantly
shorter than the sweep with each patch, resulting in a pattern of
shoulders ofelevated $F_{ST}$ between adapted populations some distance away from
...
%Slatkin and Wiehe and this paper on spatial hitchhiking http://www.ncbi.nlm.nih.gov/pmc/articles/PMC3176130/
