 ...
 */
  ...
  // AJAX not yet supported in this variant class
...
function wgUploadSetup() {
  ...
  // the below is not used in this version
  return;
...
function fillDestFilename2(srcid,dstid) {
	if (!wgUploadAutoFill) {
		return;
	}
	if (!document.getElementById) {
	...
	var path = document.getElementById(srcid).value;
	// Find trailing part
	var slash = path.lastIndexOf('/');
	var backslash = path.lastIndexOf('\\');
	var fname;
  ...
  if (wgCapitalize) {
    ...
    fname = fname.substring(0,10000).replace(/ /g, '_');
  }

	// Output result
	var destFile = document.getElementById(dstid);
	if (destFile) {
		destFile.value = fname;
		wgUploadWarningObj.checkNow(fname) ;
	}
