...
In this section, we will relax the assumption that the uniform is homogenous.
The model we analyze in section \ref{ss:discretedemes} is a model of discrete demes exchanging rare migrants,
but since the object of this paper is to work in continuous space,
we will first in sections \ref{ss:patchymutation} and \ref{ss:patchymigration} describe how a model with {\em patchy} selection can be approximated by such a discrete model.
...
We have treated the time during which the selected allele is at intermediate frequency in a deme (about $log(2N)/s$ generations) as negligible, 
...
then at time $t$, migrants from the occupied deme pass the selected allele to the unoccupied deme at rate $2 n(t) m p_f(s)$,
while newly arisen mutants arise and fix in the unoccupied deme at constant rate $2 N \mu p_f(s)$.
...
One is that the final distribution of types is no longer independent of $s$, which enters through the dynamics $n(t)$.
...
This occurs when the effective migration rate is much smaller than the mutation rate, namely...
 ...
 \begin{figure}[ht]
 \begin{center}
 %\includegraphics{discrete-by-mutrate.eps}
 %\includegraphics{discrete-by-migrprob.eps}
 \includegraphics{discrete-by-ratio.eps}
 \filenote{discrete-by-ratio.eps}
 \caption{ %
...
We have shown that in the model of discrete demes connected by weak migration, the final distribution of types depends only on the ratio of mutation to migration rates,
and that the distribution itself has a Dirichlet distribution.  
In particular, the number of independently arisen types follows the Ewens distribution, with parameter $\mu/m$.
In Figure \ref{fig:discbyratio} we plot mean number of types and size (number of demes occupied) of a sampled type,
for a model with 20 interconnected demes, across different parameter values.  
This indicates that we need the population-scaled mutation and migration rates to be within a factor of about 100 of each other for parallel adaptation to leave an interesting pattern.  
