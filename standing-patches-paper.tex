...
while the influx due to migration between patches separated by distance $R$ is on the order $2 \rho A \exp(- |s_m R/\sigma|)$. 
If the input from mutation and migration are comparable, i.e.\ if $\mu \approx \exp( -|s_m R/\sigma| )$,
then adaptation is expected to take place both by migration and mutation.
If we take $\mu = 10^{-8}$, then this is satisfied if $R/\sigma \approx 18/s_m$.
so if the disadvantage to the new mutation between patches is fairly strong (say, $s_m=.05$),
then the distance between patches must be several hundreds of times the dispersal distance;
while if $s_m$ is much smaller, say $s_m = .001$, 
then the patches must be separated by tens of thousands of times the dispersal distance.
...
The general picture is that for species with spatial structure but without extreme barriers to dispersal, 
patches will not be completely isolated, but some parallel adaptation is likely.
