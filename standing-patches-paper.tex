...
%    s_d^{-1} (\sqrt{2s_b} \chi)^2 + \sigma (\sqrt{2s_b} \chi)^3 = (\pi \rho \mu)^{-1} .
\left( \chi s_d \sigma \sqrt{2s_b} \right)^2 + \left( \chi s_d \sigma \sqrt{2s_b} \right)^3 
        = (s_d^3 \sigma^2)/(\pi \rho \mu).
\end{equation}
From this we see that $\chi$ increases {\em linearly} with $1/\sqrt{s_b}$,
...
If $\sigma^3 \s_d^3/(\pi \rho \mu)$ is small, then $\chi \approx \sqrt{ s_d / (2 s_b \pi \rho \mu)} = 1/\sqrt{\pi \lambda_0}$,
% + O( \gamma ) with $\gamma = \sigma^3 \s_d^3/(\pi \rho \mu)$
and nearly all mutants come from standing variation (note that $\sigma$ does not enter into it).
